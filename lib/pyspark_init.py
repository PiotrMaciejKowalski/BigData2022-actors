...
def string_to_array(df, list_columns_names, p):
    ...
    for df_column in list_columns:
        ...
        df_column = df_column.select("*").withColumn(
            "id1", monotonically_increasing_id()
        )
...
def load_data(spark: SparkSession) -> DataFrame:
    ...
    for column in ["knownForTitles"]:
        df_name_basics = df_name_basics.withColumn(
            column,
            when(df_name_basics[column] == "\\N", None).otherwise(
                df_name_basics[column]
            ),
        )
    for column in ["titleType", "originalTitle", "genres"]:
        df_title_basics = df_title_basics.withColumn(
            column,
            when(df_title_basics[column] == "\\N", None).otherwise(
                df_title_basics[column]
            ),
        )
    for column in ["ordering", "category", "characters"]:
        df_title_principals = df_title_principals.withColumn(
            column,
            when(df_title_principals[column] == "\\N", None).otherwise(
                df_title_principals[column]
            ),
        )

    df_title_basics = df_title_basics.withColumn(
        "isAdult", df_title_basics["isAdult"].cast(BooleanType())
    )

    df_name_basics = string_to_array(
        df_name_basics, ["primaryProfession", "knownForTitles"], ","
    )
    ...
    data = df_title_basics_selected.join(
        df_title_principals_selected, "tconst", "right"
    )
...
def add_kaggle_data(spark: SparkSession, data: DataFrame) -> DataFrame:
    ...
    oscars = (
        spark.read.option("header", "true")
        .schema(schemat_oscars)
        .csv("the_oscar_award.csv")
    )
    globe = (
        spark.read.option("header", "true")
        .schema(schemat_globe)
        .csv("golden_globe_awards.csv")
    )
    ...
    emmy_awards = (
        spark.read.option("header", "true")
        .schema(schemat_emmy_awards)
        .csv("the_emmy_awards.csv")
    )
