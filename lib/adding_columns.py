...
class Normalized_column(Transformer):
    ...
    def _transform(self, data):
        unlist = udf(
            lambda x: round(float(list(x)[0]), 3), DoubleType()
        )  # zamiana typu wartości w kolumnie z wektora na Double
        return data.withColumn(
            self.inputCol + "_norm", unlist(self.inputCol + "_norm")
        ).drop(self.inputCol + "_Vect")
...
def add_normalized_columns(data: DataFrame) -> DataFrame:
    assert (
        ...
        for x in [
            "no_nominations_oscars_norm",
            "no_oscars_norm",
            "no_nominations_globes_norm",
            "no_globes_norm",
            "no_nominations_emmy_norm",
            "no_emmy_norm",
            "no_films_norm",
            "average_films_rating_norm",
        ]
    )
    to_be_normalized = [
        "no_nominations_oscars",
        "no_oscars",
        "no_nominations_globes",
        "no_globes",
        "no_nominations_emmy",
        "no_emmy",
        "no_films",
        "average_films_rating",
    ]
    for i in to_be_normalized:
        assembler = VectorAssembler(
            inputCols=[i], outputCol=i + "_Vect"
        )  # zamiana wartości z bigint na wektory
        scaler = MinMaxScaler(
            inputCol=i + "_Vect", outputCol=i + "_norm"
        )  # przeskalowanie wartości (funkcja działa na liście wektorów utworzonej z wartości w skalowanej kolumnie)
...
def add_top_type(data: DataFrame) -> DataFrame:
    ...
    aggregated_table = (
        df3.withColumn("count", count("*").over(w))
        .withColumn("rn", row_number().over(w.orderBy(desc("count"))))
        .sort(desc("count"))
        .filter("rn = 1")
        .groupBy("nconst")
        .agg(first("titleType").alias("top_type"))
    )
...
def add_top_genres(data: DataFrame) -> DataFrame:
    ...
    aggregated_table_2 = (
        df5.withColumn("count", count("*").over(w_2))
        .withColumn("rn", row_number().over(w_2.orderBy(desc("count"))))
        .sort(desc("count"))
        .filter("rn = 1")
        .groupBy("nconst")
        .agg(first("genres").alias("top_genres"))
    )
...
def add_all_columns(spark: SparkSession, data: DataFrame) -> DataFrame:
    ...
    data = add_average_films_ratings(
        spark, data
    ).cache()  # .cache() przyspieszy działanie kolejnej funkcji - dane po wykonaniu funkcji add_average_films_ratings są zapamiętywane i przechowywane, dzięki czemu przy kolejnej funkcji wszystkie wcześniejsze operacje nie muszą być wykonywane przy każdej komendzie wymagającej tych danych
